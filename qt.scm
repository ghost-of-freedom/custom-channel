          ...
          (lambda* (#:key outputs #:allow-other-keys)
            (let* ((out (assoc-ref outputs "out")))
              (for-each (lambda (file)
                          (substitute* file
                            (("/gnu/store/[a-zA-Z0-9]+-qtbase-[0-9.]+/lib/cmake/(.*)$" all sub)
                            (string-append out "/lib/cmake/" sub))
            ...
            #t)))))
   (inputs (list qtbase-5 gtk+ gtk+-2 libx11 pango))
   (native-inputs (list qtbase-5 gtk+ gtk+-2 libx11 pango))
